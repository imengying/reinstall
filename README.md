...
**核心目标**：将任意 Linux 系统（VPS/云主机/物理机）一键重装为纯净的 Debian 系统，使用Btrfs文件系统，可开启Zstd 压缩，实现**30%-50% 的磁盘空间节省**和显著的 I/O 性能提升。
...
| 💾 **Btrfs + Zstd** | 手动开启 `compress=zstd`，大幅节省空间并延长闪存寿命。 |
...
> [!TIP]
> **关于 Btrfs 压缩**：
> 若在 DD 系统后需开启透明压缩，请手动编辑 `/etc/fstab` 添加 `compress=zstd` 挂载选项，重启后执行全盘压缩：
